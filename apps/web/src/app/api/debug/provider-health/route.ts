...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  if (parsedHealth) {
    const lastSuccessMs = parsedHealth.lastSuccessAt
      ? Date.parse(parsedHealth.lastSuccessAt)
      : null;
    const lastErrorMs = parsedHealth.lastErrorAt ? Date.parse(parsedHealth.lastErrorAt) : null;

    const analysis: Record<string, unknown> = {
      lastSuccessAge: lastSuccessMs
        ? `${Math.round((nowMs - lastSuccessMs) / 60000)} minutes ago`
        : 'never',
      lastErrorAge: lastErrorMs
        ? `${Math.round((nowMs - lastErrorMs) / 60000)} minutes ago`
        : 'never',
  ...
  if (!redis) {
    diagnostics.conclusion =
      'UPSTASH NOT CONFIGURED: Redis client cannot be created. Provider health cannot be read or written.';
  } else if (rawHealthKey === null) {
    diagnostics.conclusion =
      'KEY MISSING: Cron jobs have never written to the health key. Check if crons are running and completing successfully.';
  } else if (derivedStatus === 'degraded') {
    diagnostics.conclusion =
      'DEGRADED: Health key exists but conditions trigger degraded status. See healthAnalysis for details.';
  } else if (derivedStatus === 'down') {
