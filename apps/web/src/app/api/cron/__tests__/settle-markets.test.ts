...
describe('findCityByBytes32', () => {
  it('finds city by matching hash', async () => {
    ...
    const cities = [
      { slug: 'nyc', name: 'New York', latitude: 40.7, longitude: -74.0 },
      { slug: 'la', name: 'Los Angeles', latitude: 34.0, longitude: -118.2 },
    ];

    const nycHash = keccak256(toBytes('nyc'));
    const found = cities.find(c => keccak256(toBytes(c.slug)) === nycHash);

    expect(found?.slug).toBe('nyc');
  ...
  it('returns null for unknown hash', async () => {
    ...
    const found = cities.find(c => keccak256(toBytes(c.slug)) === unknownHash);
