...
function parseProviderHealth(raw: unknown): ProviderHealth | null {
  try {
    ...
    if (!result.success) {
      console.warn(
        `${LOG_PREFIX} Failed to validate health data: ${result.error.message}`
      );
      return null;
    }
    return result.data;
...
async function updateProviderHealth(
  ...
  try {
    ...
    console.log(
      `${LOG_PREFIX} Recorded ${operation}: recentErrors=${updated.recentErrors}`
    );
  } catch (error) {
...
export async function readProviderHealth(): Promise<ProviderHealth | null> {
  ...
  if (!redis) {
    console.warn(
      `${LOG_PREFIX} Cannot read health: Upstash Redis not configured`
    );
    return null;
  }
  ...
  try {
    ...
    if (!raw) {
      console.warn(
        `${LOG_PREFIX} Health key not found: ${REDIS_KEYS.PROVIDER_HEALTH}`
      );
      return null;
    }
