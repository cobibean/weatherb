...
export type SettlementResult = {
  marketId: string;
  action: 'pending' | 'reconciled' | 'settled' | 'cancelled';
  transactionHash?: Hex;
};
...
export async function settleMarket(
  ...
  if (isTerminal(market)) {
    ...
    return { marketId: id.toString(), action: 'reconciled' as const };
  }
  const target = Number(market.resolveTime);
  const now = Math.floor(Date.now() / 1000);
  if (now < target) return { marketId: id.toString(), action: 'pending' as const };
