 ...
 * It prepares an attestation request using Tomorrow.io weather data.
 *
 * Usage:
...
const PUBLIC_VERIFIER_HEADER_VALUE = ['00000000', '0000', '0000', '0000', '000000000000'].join('-');
...
async function main() {
  ...
  const cityIdBytes32 = keccak256(toBytes(city.slug));

  console.log(`📍 City: ${city.slug} → ${cityIdBytes32}`);
  ...
  if (!apiKey) {
    console.log('   ❌ TOMORROW_IO_API_KEY is required to test Tomorrow.io');
    process.exit(1);
  }

  // Step 1: Test Tomorrow.io API directly
  console.log('───────────────────────────────────────────────────────────────');
  console.log('  Step 1: Test Tomorrow.io API');
  console.log('───────────────────────────────────────────────────────────────');

  const tomorrowUrl = `https://api.tomorrow.io/v4/weather/realtime?location=${city.latitude},${city.longitude}&apikey=${apiKey}`;
  const redactedTomorrowUrl = tomorrowUrl.replace(apiKey, '[redacted]');
  console.log(`   URL: ${redactedTomorrowUrl}`);

  try {
    const weatherResponse = await axios.get(tomorrowUrl, {
      headers: { 'Accept': 'application/json' },
    });

    const observedTime = weatherResponse.data.data.time;
    const tempC = weatherResponse.data.data.values.temperature;
    const tempF = (tempC * 9) / 5 + 32;
    const tempFTenths = Math.floor(tempF * 10);

    console.log(`   ✅ Tomorrow.io API working!`);
    console.log(`   Time: ${observedTime}`);
    console.log(`   Temperature: ${tempC}°C = ${tempF.toFixed(1)}°F = ${tempFTenths} tenths`);
  } catch (error) {
    console.log(`   ❌ Tomorrow.io API failed: ${error}`);
    process.exit(1);
  }
  ...
  const postProcessJq = `.data | { cityId: "${cityIdBytes32}", observedTimestamp: (.time | fromdateiso8601), tempTenths: (((.values.temperature * 9 / 5 + 32) * 10) | floor) }`;
    ...
    const verifierResponse = await axios.post(
      ...
      payload,
      {
        headers: {
          ...
          'X-API-KEY': PUBLIC_VERIFIER_HEADER_VALUE,
        },
        timeout: 30000,
      },
    );
