...
**Implementation:** QStash schedules per-market settlement at resolve time, with a 5-minute cron fallback window

- Settlement service is the only entity that resolves markets in V1 (trusted settler pattern)
- Fetches actual temperature from weather provider (Tomorrow.io)
...
### ✅ Completed (Epics 0-6)
- **Epic 0-3:** Monorepo, contracts, weather provider (Tomorrow.io), settlement
...
- **Tomorrow.io** as primary provider (realtime API with caching)
