 ...
 * Reason: Tests written for older Epic 8 API and divergent WeeklyMetrics types
 * 
 * ERRORS (77):
 * - Uses sendWeeklyReportEmail (renamed to sendWeeklySummaryEmail)
 * - Uses old WeeklyMetrics shape with totalSuggestions, newSuggestions
 * - Different WeeklyMetrics type in metrics.ts vs ai-insights.ts
 * - Missing date-fns dependency for imports
 * - Uses generateAIInsights (now generateWeeklyInsights)
 * 
 * TO REBUILD THIS TEST:
 * 1. Consolidate WeeklyMetrics types (metrics.ts vs ai-insights.ts have diverged)
 * 2. Update email function imports to current names
 * 3. Add date-fns to dependencies OR remove date-fns usage
 * 4. Update metric collection expectations to current schema
 * 5. Review ai-insights.ts for current API
 * 
 * WHAT THIS TESTED:
 * - Weekly metrics collection from database
 * - AI-powered insights generation via OpenAI
 * - Email composition with metrics, insights, and magic links
...
vi.mock('process', () => ({
  env: {
    ...
    ADMIN_EMAIL: 'admin@weatherb.app',
  },
}));
