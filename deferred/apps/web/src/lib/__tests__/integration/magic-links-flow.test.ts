 ...
 * 
 * ERRORS (55+):
 * - Uses old export names (validateMagicLink, sendApprovalEmail, sendDenialEmail, sendWeeklyReportEmail)
 * - Uses old Prisma schema fields (city, votes, token on MagicLink)
 * - Uses lowercase enum literals (pending/approved vs PENDING/APPROVED)
 * - Tests expect {token, url} return shape but generateMagicLink now returns string
 * 
 * TO REBUILD THIS TEST:
 * 1. Check current exports in: magic-links.ts, email.ts
 * 2. Update mock fixtures to match current Prisma schema (Suggestion, MagicLink)
 * 3. Update enum values to uppercase (PENDING, APPROVED, DENIED)
 * 4. Update return type expectations for generateMagicLink
 * 5. Review test-runner.ts for current startTestWindow signature
 * 
 * WHAT THIS TESTED:
 * - Magic link generation with secure hashed tokens
 * - Email sending with approve/deny links
 * - One-time use validation (prevents replay attacks)
 * - Expiry enforcement
 * - Complete flow: email click → suggestion status update → test window start
 * 
