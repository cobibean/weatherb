 ...
 * 
 * ERRORS (similar to test-window-mock.test.ts):
 * - Uses marketId instead of contractMarketId
 * - Old return type expectations for test runner functions
 * - Missing array index guards for strict null checks
 * 
 * TO REBUILD THIS TEST:
 * 1. See test-window-mock.test.ts.disabled for similar fixes
 * 2. Update all market property accesses
 * 3. Update function return type expectations
 * 
 * WHAT THIS TESTED:
 * - Full integration test of Epic 8 test window flow
 * - Suggestion approval through test completion
 * - Real contract interaction mocking
 * 
...
const createMockTestRun = (overrides?: Partial<TestRun>): TestRun => ({
  ...
  marketsCreated: 5,
  marketsSettled: 0,
  fundingAmount: new Prisma.Decimal('15.0'),
  ...
  describe('Complete Test Window Flow', () => {
    it('should execute the full test window lifecycle successfully', async () => {
      ...
      mockPrismaClient.testRun.update.mockResolvedValue({
        ...
        recoveredAmount: new Prisma.Decimal('14.5'),
        netCost: new Prisma.Decimal('0.5'),
        keysDisposed: true,
        payoutVerified: true,
    ...
    it('should send proper email notifications at key stages', async () => {
      ...
      const mockTestRun = createMockTestRun({
        ...
        totalVolume: 10.5,
        recoveredAmount: new Prisma.Decimal('14.7'),
        netCost: new Prisma.Decimal('0.3'),
      });
