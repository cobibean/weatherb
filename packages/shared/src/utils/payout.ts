 ...
 * Uses bigint for precision with large USDC amounts.
 */
 ...
 */
export function feeFromLosingPool(losingPool: bigint, feeBps: bigint = FEE_BPS): bigint {
...
export function calculatePayoutForWinner(
  ...
  const fee = feeFromLosingPool(losingPool, feeBps);
...
export function calculatePotentialPayout(
  ...
  const payout = calculatePayoutForWinner(winningPool, losingPool, betAmount, feeBps);

  // Calculate multiplier (payout / stake)
  // Use Number for display purposes (safe for UI ranges)
  const multiplier = betAmount > 0n ? Number((payout * 1000n) / betAmount) / 1000 : 0;

  // Calculate new odds percentages
  const newYesPercent = totalPool > 0n ? Number((newYesPool * 100n) / totalPool) : 50;
...
export function getImpliedMultipliers(
  ...
  const yesMultiplier = yesNum > 0 ? 1 + (noNum * (1 - Number(feeBps) / 10_000)) / yesNum : 99.0; // Massive payout if you're first

  // NO multiplier: if NO wins, you get your stake + share of YES pool
  const noMultiplier = noNum > 0 ? 1 + (yesNum * (1 - Number(feeBps) / 10_000)) / noNum : 99.0;
...
/**
 * Format a bigint USDC amount for display.
...
/** @deprecated Legacy callers only. Active Arc UI uses formatUsdc. */
export const formatFlr = formatUsdc;
